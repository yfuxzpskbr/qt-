...
#include "ui_DifficultPattern.h"
const int WIN = 1;
const int LOSE = 0;

DifficultPattern::DifficultPattern(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::DifficultPattern)
{
    ui->setupUi(this);
    ///设置固定大小
    this->setFixedSize(1280,705);

    ///设置图标1和标题
    this->setWindowIcon(QIcon(":/res/MineClearance.png"));
    this->setWindowTitle("困难模式");
    ...
    ///实现返回
    connect(backBtn,&QPushButton::clicked,this,&DifficultPattern::back);
...
//    qDebug()<<"创建地图用时："<<end-start<<"ms";
    ...
    ///监听点击信号
    connect(this,&DifficultPattern::click,this,[=](){
        sweep(clickedBtn,clickedi,clickedj,true);
    });

    ///监听游戏结束信号
    connect(this,&DifficultPattern::gameOver,[=](int Case){
...
DifficultPattern::~DifficultPattern()
{
    delete ui;
}
 ...
 */
void DifficultPattern::paintEvent(QPaintEvent *)
{
    QPainter painter(this);
    QPixmap pix;
    pix.load(":/res/EasyBg.png");
    pix = pix .scaled(1280,685);
 ...
 */
void DifficultPattern::creatMine()
{
    qsrand((uint)time(0));
    while(mineNumber<120){
        int i = qrand()%20;
        int j = qrand()%40;
 ...
 */
int DifficultPattern::checkMine(int i, int j)
{
    int cnt=0;
    if(i-1>=0){
        cnt = cnt + (map[i-1][j]==8?1:0);
        if(j-1>=0)cnt = cnt + (map[i-1][j-1]==8?1:0);
        if(j+1<40)cnt = cnt + (map[i-1][j+1]==8?1:0);
    }
    cnt = cnt + (map[i][j]==8?1:0);
    if(j-1>=0)cnt = cnt + (map[i][j-1]==8?1:0);
    if(j+1<40)cnt = cnt + (map[i][j+1]==8?1:0);
    if(i+1<20){
        cnt = cnt + (map[i+1][j]==8?1:0);
        if(j-1>=0)cnt = cnt + (map[i+1][j-1]==8?1:0);
        if(j+1<40)cnt = cnt + (map[i+1][j+1]==8?1:0);
 ...
 */
bool DifficultPattern::iscontain(QVector<int> p, QVector<int> q, int m, int n)
{
...
void DifficultPattern::test()
{
    for(int i=0;i<20;i++){
            qDebug()<<map[i][0]<<map[i][1]<<map[i][2]<<map[i][3]<<map[i][4]
                    ...
                    <<map[i][25]<<map[i][26]<<map[i][27]<<map[i][28]<<map[i][29]
                    <<map[i][30]<<map[i][31]<<map[i][32]<<map[i][33]<<map[i][34]
                    <<map[i][35]<<map[i][36]<<map[i][37]<<map[i][38]<<map[i][39];
    }
}
 ...
 * @param click 是否是手动点击
 */
void DifficultPattern::sweep(int clickedBtn, int i, int j, bool click)
{
 ...
 */
void DifficultPattern::mousePressEvent(QMouseEvent *event)
{
    if(event->x()>=0 && event->x()<=1280 && event->y()>=45 && event->y()<=685){
        if(!isgameover){
 ...
 * @param pic 图标代码
 */
void DifficultPattern::setLabelIcon(QLabel *label, int pic)
{
    QPixmap pix;
    switch(pic){
        case 0:
 ...
 */
bool DifficultPattern::isVisOver()
{
    for(int i=0;i<20;i++){
        for(int j=0;j<40;j++){
 ...
 * @return 游戏结束返回true
 */
bool DifficultPattern::isGameOver()
{
    if(isgameover)return true;
    if((mineNumber==0&&flagNumber==120) || isVisOver()){
        timer1->stop();
//        qDebug()<<"isgameover:"<<isgameover;
 ...
 * @param a 待更新标签
 */
void DifficultPattern::updateLabel1(QLabel *a)
{
 ...
 * @param b 待更新标签
 */
void DifficultPattern::updateLabel2(QLabel *b)
{
