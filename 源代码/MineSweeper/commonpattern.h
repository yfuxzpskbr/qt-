#ifndef COMMONPATTERN_H
#define COMMONPATTERN_H
...
class CommonPattern : public QMainWindow
{
    Q_OBJECT

public:
    explicit CommonPattern(QWidget *parent = 0);
    ~CommonPattern();
    void paintEvent(QPaintEvent *);//覆写绘图事件
    void creatMine();//随机生成60个地雷
...
private:
    Ui::CommonPattern *ui;
    int map[20][20];//地图数组
    int vis[20][20];//标记地图是否被访问  -1表示已经翻开，0表示未翻开，1表示插旗状态、2表示问号状态
    QVector<int> row;//存储地雷所在行
    QVector<int> col;//存储地雷所在列
    QLabel *mapLabel[20][20];//地图标签
    int mineNumber=0;//地雷个数 一般模式60个
