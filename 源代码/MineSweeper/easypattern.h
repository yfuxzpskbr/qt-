#ifndef EASYPATTERN_H
#define EASYPATTERN_H
...
class EasyPattern : public QMainWindow
{
    Q_OBJECT

public:
    explicit EasyPattern(QWidget *parent = 0);
    ~EasyPattern();
    void paintEvent(QPaintEvent *); //覆写绘图事件
    void creatMine();   //随机生成15个地雷
...
private:
    Ui::EasyPattern *ui;
    int map[10][10];    //地图数组
    int vis[10][10];    //标记地图是否被访问  -1表示已经翻开，0表示未翻开，1表示插旗状态、2表示问号状态
    QVector<int> row;   //存储地雷所在行
    QVector<int> col;   //存储地雷所在列
    QLabel *mapLabel[10][10];   //地图标签
    int mineNumber=0;      //地雷个数 简单模式15个
